...
from model.pytorch_grad_cam.utils.image import scale_cam_image
from model.pytorch_grad_cam.utils.model_targets import ClassifierOutputTarget
...
class BaseCAM:
    ...
    def forward(
        ...
        if targets is None:
            target_categories = np.argmax(outputs.cpu().detach().float().numpy(), axis=-1)
    ...
    def compute_cam_per_layer(
        ...
        if self.detach:
            activations_list = [a.cpu().detach().float().data.numpy() for a in self.activations_and_grads.activations]
            grads_list = [g.cpu().detach().float().data.numpy() for g in self.activations_and_grads.gradients]
        else:
