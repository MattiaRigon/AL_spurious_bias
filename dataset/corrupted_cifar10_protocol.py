'''
from https://github.com/alinlab/LfF/blob/master/data/corrupted_cifar10_protocol.py
'''
# -*- coding: utf-8 -*-
# flake8: noqa
...
def glass_blur(x, severity=1):
    ...
    x = np.uint8(
        gaussian(np.array(x) / 255.0, sigma=c[0], channel_axis=-1) * 255
    )
    ...
    return (
        np.clip(gaussian(x / 255.0, sigma=c[0], channel_axis=-1), 0, 1) * 255
    )
